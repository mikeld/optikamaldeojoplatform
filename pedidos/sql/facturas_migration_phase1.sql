...
SET @sql = IF(@col_exists = 0,
    'ALTER TABLE `facturas_audits` ADD COLUMN `invoice_subtotal` DECIMAL(10, 2) DEFAULT 0.00 COMMENT ''Invoice subtotal before taxes'' AFTER `invoice_number`',
...
SET @sql = IF(@col_exists = 0,
    'ALTER TABLE `facturas_audits` ADD COLUMN `tax_total` DECIMAL(10, 2) DEFAULT 0.00 COMMENT ''Total tax amount'' AFTER `invoice_subtotal`',
